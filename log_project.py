...
def show_protocol():
        log_treeview.delete(*log_treeview.get_children())
        protocol_dict = root.protocol_dict

        for k, v in protocol_dict.items():
            for i in range(1, len(v['date'])):
                # if를 반복문에서 뺄 수 없나. 비효율적이네
                if value1.get() & (k=='POP3D'):
                    log_treeview.insert('', 'end', text=k, values=(v['date'][i], v['time'][i], v['ip'][i], v['message'][i]))
                if value2.get() & (k=='SMTPC'):
                    log_treeview.insert('', 'end', text=k, values=(v['date'][i], v['time'][i], v['ip'][i], v['message'][i]))
                if value3.get() & (k=='SMTPD'):
                    log_treeview.insert('', 'end', text=k, values=(v['date'][i], v['time'][i], v['ip'][i], v['message'][i]))
